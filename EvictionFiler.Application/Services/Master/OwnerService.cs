...
namespace EvictionFiler.Application.Services.Master
{
    ...
    {
        private readonly ITypeOwnerRepository _repository;
        private readonly IUnitOfWork _unitOfWork;
        public OwnerService(ITypeOwnerRepository repository, IUnitOfWork unitOfWork)
        {
            _repository = repository;
            _unitOfWork = unitOfWork;
        }

        public async  Task<bool> Create(EditToOwnerDto dto)
        {
            var forms = new TypeOfOwner
            {
        ...
        public async Task<PaginationDto<EditToOwnerDto>> GetAllOwnerAsync(int pageNumber, int pageSize, string searchTerm)
        {
        ...
        .ToList();
            ...
            return new PaginationDto<EditToOwnerDto>
            {
                Items = forms,
        ...
        public async Task<EditToOwnerDto?> GetOwnerByIdAsync(Guid? id)
        {
            var form = await _repository.GetAsync(id);

            if (form == null) return null;
            return new EditToOwnerDto
            {
                Id = form.Id,
                Name = form.Name,
        ...
        public async  Task<bool> UpdateOwnerAsync(EditToOwnerDto dto)
        {
