...
namespace EvictionFiler.Application.Services.Master
{
    ...
    {
        private readonly ITenancyTypeRepository _repository;
        private readonly IUnitOfWork _unitOfWork;
        public TenancyTypeService(ITenancyTypeRepository repository, IUnitOfWork unitOfWork)
        {
            _repository = repository;
            _unitOfWork = unitOfWork;
        }

        public async  Task<bool> Create(EditToTenancyTypeDto dto)
        {
            var forms = new TenancyType
            {
        ...
        public async Task<bool> DeleteTenancyTypeAsync(Guid id)
        {
        ...
        public async Task<PaginationDto<EditToTenancyTypeDto>> GetAllTenancyTypeAsync(int pageNumber, int pageSize, string searchTerm)
        {
        ...
        .Take(pageSize)
        .Select(x => new EditToTenancyTypeDto
        {
            Id = x.Id,
            Name = x.Name,
        ...
        .ToList();
            ...
            return new PaginationDto<EditToTenancyTypeDto>
            {
                Items = forms,
        ...
        public async Task<EditToTenancyTypeDto?> GetTenancyTypeByIdAsync(Guid? id)
        {
            var form = await _repository.GetAsync(id);

            if (form == null) return null;
            return new EditToTenancyTypeDto
            {
                Id = form.Id,
                Name = form.Name,
        ...
        public async  Task<bool> UpdateTenancyTypeAsync(EditToTenancyTypeDto dto)
        {
